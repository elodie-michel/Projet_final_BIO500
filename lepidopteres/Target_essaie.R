...
list(
  tar_target(
    name = fichiers_csv,
    command = list.files("./donnees", pattern = "\\.csv$", full.names = TRUE),
    format = "file"
  ),
  tar_target(
    name = g_b_brut,
    command = traiter_donnees(fichiers_csv)
  ),
  
  tar_target(
    name = g_b_nettoye,
    command = Col_supprimé_modifié(g_b_brut)
  ),
  tar_target(
    name = g_b_final,
  ...
  tar_target(
    name = base_sqlite,
    command = creer_base_de_donnees_SQL(
      nom_fichier = "lepidopteres.db",
      table_obs = table_obs,
